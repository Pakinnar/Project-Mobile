          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  widget.contact.type == 'user_admin' ? 'แชตกับแอดมิน' : 'แชตส่วนตัว',
                  style: const TextStyle(
                    fontSize: 12,
...
class ChatRoomApiService {
  ...
  static Future<List<UserChatMessage>> getMessages(int conversationId) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('โหลดข้อความไม่สำเร็จ (${response.statusCode}) ${response.body}');
    }
  ...
  static Future<UserChatMessage> sendMessage({
    ...
    if (response.statusCode != 201) {
      throw Exception('ส่งข้อความไม่สำเร็จ (${response.statusCode}) ${response.body}');
    }
