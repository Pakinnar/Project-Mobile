...
class _AddJobPageState extends State<AddJobPage> {
  ...
  Future<String> _getGoogleFormattedAddress(LatLng pos) async {
    final url = Uri.parse(
      'http://localhost:3000/api/maps/reverse-geocode?lat=${pos.latitude}&lng=${pos.longitude}',
    );
  ...
  Future<void> _searchLocation(String query) async {
    ...
    try {
      final url = Uri.parse(
        'http://localhost:3000/api/maps/search-geocode?q=${Uri.encodeComponent(query)}',
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('ไม่พบสถานที่ที่ค้นหา ($e)')),
        );
      }
    } finally {
  ...
  Future<void> _submitJob() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('ลงประกาศงานไม่สำเร็จ: $e')),
      );
    } finally {
...
class _FullMapPageState extends State<_FullMapPage> {
  ...
  Future<void> _search(String q) async {
    ...
    try {
      final url = Uri.parse(
        'http://localhost:3000/api/maps/search-geocode?q=${Uri.encodeComponent(q)}',
      );
